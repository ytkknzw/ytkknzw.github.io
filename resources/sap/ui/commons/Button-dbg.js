...
function(jQuery, library, Control, EnabledPropagator, IconPool) { "use strict";
	 ...
	 */
	var Button = Control.extend("sap.ui.commons.Button", /** @lends sap.ui.commons.Button.prototype */ 
		{ metadata : {
		...
		properties : {
			...
			/**
			 *
			 * Boolean property to enable the control (default is true). Buttons that are disabled 
			 * have other colors than enabled ones, depending on custom settings.
			 */
			...
			/**
			 * Icon to be displayed as graphical element within the button when it is hovered 
			 * (only if also a base icon was specified). If not specified the base icon is used.
			 * If a icon font icon is used, this property is ignored.
			 */
			iconHovered : {type : "sap.ui.core.URI", group : "Appearance", defaultValue : ''},

			/**
			 * Icon to be displayed as graphical element within the button when it is selected
			 * (only if also a base icon was specified). If not specified the base or hovered icon is used.
			 * If a icon font icon is used, this property is ignored.
			 */
			...
			/**
			 * Specifies the button height. If this property is set, the height which is specified by 
			 * the underlying theme is not used any longer.
			 */
			height : {type : "sap.ui.core.CSSSize", group : "Dimension", defaultValue : null},

			/**
			 * Indicates if the button is styled. If not it is rendered as native HTML-button. 
			 * In this case a custom styling can be added usig addStyleClass.
			 */
			 ...
			 * (e.g. emphasized)
			 */
			style : {type : "sap.ui.commons.ButtonStyle", group : "Appearance", 
				defaultValue : sap.ui.commons.ButtonStyle.Default}
		},
		associations : {
			...
			/**
			 * Association to controls / ids which label this control 
			 * (see WAI-ARIA attribute aria-labelledby).
			 */
	...
	Button.prototype.handleMouseDown = function(oEvent, bFocus) {
		...
		if (bFocus && (!!sap.ui.Device.browser.webkit || 
			(!!sap.ui.Device.browser.firefox && navigator.platform.indexOf("Mac") === 0))) {
			if (sap.ui.Device.browser.mobile && !!sap.ui.Device.browser.webkit) {
				//In mobile Webkit Browsers (IPad) the focus must be set immediately to ensure 
				//that a focusout happens whereever the focus currently is. The deleayedCall below is 
				//still needed due to the reason described above. (CSN 2536817 2012)
				this.focus();
			}
	...
	Button.prototype.getAccessibilityInfo = function() {
		...
		return {
			role: "button",
			type: sap.ui.getCore()
				.getLibraryResourceBundle("sap.ui.commons")
